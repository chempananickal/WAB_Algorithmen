...
The objective of this paper was to compare the performance of two algorithms, the \textit{\acrlonglc{sam}} and the \textit{\acrlonglc{esa}}, 
for solving the \acrlonglc{lcs} problem \textbf{between two strings} in a pure Python implementation.
...
The results showed that \textbf{so long as one is programming in pure Python, and so long as only two strings are involved}, an \acrlonglc{esa} offers practically zero advantages over a \acrlonglc{sam}.
The \acrlonglc{sam} was substantially faster to build and to query in all but one scenario, 
and although the \acrlonglc{esa} had an overall smaller index size, this would do very little in real-life memory-constrained applications, 
as its peak memory usage was either comparable to the \acrlonglc{sam} or only barely smaller in all scenarios and string lengths.
Not to mention that the \acrlonglc{sam} is much simpler to implement.
...
Due to Python's memory management, the \acrlonglc{esa} would also lose its cache locality, which is the main reason for its compactness in lower level languages like C.
...
Auch von Vorteil ist, dass der \acrlong{sam} viel einfacher zu implementieren ist.
