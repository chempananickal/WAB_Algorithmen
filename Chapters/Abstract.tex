...
for solving the \acrlong{lcs} problem in a pure Python implementation.
...
The results showed that \textbf{so long as one is programming in pure Python}, an \acrlong{esa} offers practically zero advantages over a \acrlong{sam}.
The \acrlong{sam} was substantially faster to build and to query, 
...
as its peak memory usage was much higher than that of the \acrlong{sam} across all scenarios and string lengths.
...
leading to even simple data types like integers and lists having a much higher memory overhead than in lower level languages like C.
...
bei der Lösung des \acrlong{lcs}-Problems in einer reinen Python-Implementierung zu vergleichen.
...
Die Ergebnisse zeigten, dass \textbf{solange man in reinem Python programmiert}, ein \acrlong{esa} praktisch keine Vorteile gegenüber einem \acrlong{sam} bietet.
Der \acrlong{sam} war wesentlich schneller im Aufbau und in der Abfrage,
...
da sein Spitzen-Speicherverbrauch in allen Szenarien und Stringlängen viel höher war als der des \acrlong{sam}s.
...
was dazu führt, dass selbst einfache Datentypen wie Ganzzahlen und Listen einen viel höheren Speicher-Overhead haben als in Sprachen wie C.
