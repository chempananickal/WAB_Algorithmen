...
The usage of AI tools within this project is documented here, including the prompts used and the outputs received.
...
\begin{longtable}{p{0.20\textwidth} p{0.40\textwidth} p{0.40\textwidth}}
    ...
    GitHub Copilot 28 & Asked to calculate per-metric, per-scenario linear regression slope ratios (SAM slope / ESA slope) and export them as LaTeX tables and a CSV, so the hardware-independent ratios can be cited in the report text. & Added \texttt{export\_slope\_ratio\_tables} to \texttt{statistics\_io.py} using \texttt{numpy.polyfit} on median values vs.\ string length\\
    GitHub Copilot 29 & Asked to calculate the R squared values for the linear regressions and include them in the slope ratio tables. & Updated \texttt{export\_slope\_ratio\_tables} to also calculate $R^2$ using \texttt{numpy.polyval} and include it in the output tables\\
\end{longtable}
}
