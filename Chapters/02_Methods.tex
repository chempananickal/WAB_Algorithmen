...
To give the \acrfull{esa} the best chance possible, the \acrshort{sa} was constructed using the \acrfull{sais} algorithm, 
...
with a \gls{sentinel} character in between them (\texttt{\textbackslash0} in this case) to separate them.
...
The \acrshort{sa} and the \acrshort{lcp} array were then combined to form the \acrshort{esa}.

For each scenario, \textbf{25 string pairs} were generated at each of the lengths 100, 300, 600, 1000, 2000, 3000, 4500, 6000, 8000, and 10000 characters.
Each string pair was benchmarked on both algorithms \textbf{20 times} to try and ensure statistical significance of the results.
A fixed random seed of \textbf{42} was used throughout to ensure reproducibility.
