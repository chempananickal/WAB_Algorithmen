...
Slope ratio tables were also generated, which shows the ratio of the linear regression slopes of the ESA and SAM for each scenario and metric,
as a hopefully more hardware-independent measure of the relative performance of the two algorithms. These are also included in the appendix and as a CSV file in the supplementary materials.
...
In the worst case scenario (random\_uniform), the total time for the \acrshort{esa} is about 15.83 times higher than that of the \acrshort{sam}.
And even in the best case scenario (near\_identical), the \acrshort{esa} is about 7.96 times higher than that of the \acrshort{sam}!
...
In the best case scenario (near\_identical), the \acrshort{sam} has an index size that is about 2.89 times larger than that of the \acrshort{esa}, 
and in the worst case scenario (repetitive\_with\_noise), the \acrshort{sam} has an index size that is about 4.97 times larger than that of the \acrshort{esa}.
...
with the \acrshort{esa} even exhibiting slightly higher peak memory usage than the \acrshort{sam} in the random\_uniform scenario (only about 5\% higher, see Appendix for details).
...
The \acrshort{sam} had about 43\% higher peak memory usage than the \acrshort{esa} in the mutated\_implant scenario, and about 58\% higher in the repetitive\_with\_noise scenario (see Appendix for details).
