...
In this paper, two algorithms for solving the longest common substring (\acrshort{lcs}) problem between two strings were compared: the \acrfull{sam} and the \acrfull{esa}.
...
This could potentially be due to the way the Python implementation constructs the result set of matching substrings, although it warrants further investigation to determine whether this pattern persists in lower-level language implementations or with different result collection strategies.

\textit{In summary, to answer the original research question of which algorithm is more efficient for finding the longest common substring between two strings in Python, the \textbf{\acrfull{sam}} proves to be the more favorable choice given the criteria examined in this study.}
...
is a more complex data structure that maintains the same $O(|\sum_{i=1}^k n_i|)$ construction complexity as the \acrshort{esa}, where $n_i$ is the length of string $i$ and $k$ is the number of strings.
