...
Of particular interest are the \acrfull{sam} (also known as \acrfull{dawg}) and the \acrfull{sa} with \acrfull{lcp} Array, which both yield the search result in linear time, but differ in their construction time and space requirements.

\section*{Research Question}
\addcontentsline{toc}{section}{Research Question}
The primary research question of this paper is:
\begin{quote}
  How do the \acrlong{sam} and the \acrfull{sa} with \acrshort{lcp} Array compare in terms of time and space complexity when solving the \acrlong{lcs} problem in an everyday programming context?
\end{quote}
...
\begin{itemize}
    \item To implement the \acrlong{sam} and the \acrfull{sa} with \acrfull{lcp} Array algorithms for solving the \acrlong{lcs} problem.
...
The \acrshort{sam} and \acrshort{sa} + \acrshort{lcp} Array algorithms will be implemented in Python, and only synthetic test cases will be used to evaluate their performance.
The amount of memory consumed by the algorithms will be measured using the \texttt{memory\_profiler} library, while the time taken by the algorithms will be measured using the \texttt{timeit} library.
